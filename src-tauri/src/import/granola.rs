...
impl ImportSummary {
    pub fn message(&self) -> String {
        let mut parts = vec![if self.imported > 0 {
            format!("Imported {} meeting{}", self.imported, if self.imported == 1 { "" } else { "s" })
        } else {
        ...
        if self.skipped_rows > 0 {
            parts.push(format!("{} row{} skipped", self.skipped_rows, if self.skipped_rows == 1 { "" } else { "s" }));
        }
        parts.join(" · ")
    }
}
...
const START_HEADERS: &[&str] = &["starttime", "meetingstart", "meetingdate", "date", "createdat", "created", "scheduledstarttime", "calendareventstart"];
const END_HEADERS: &[&str] = &["endtime", "meetingend", "updatedat", "updated", "scheduledendtime", "calendareventend"];
const SUMMARY_HEADERS: &[&str] = &["summarytext", "summary", "notesummary", "aisummary"];
const NOTES_HEADERS: &[&str] = &["notes", "notemarkdown", "notesmarkdown", "summarymarkdown", "enhancednotes", "privatenotes"];
const TRANSCRIPT_HEADERS: &[&str] = &["transcript", "rawtranscript", "fulltranscript", "transcription"];
...
fn normalize_header(h: &str) -> String {
    h.trim_start_matches('\u{feff}').to_lowercase().chars().filter(|c| c.is_alphanumeric()).collect()
}
...
pub fn parse_csv_rows(text: &str) -> Result<Vec<Vec<String>>, String> {
    ...
    Ok(rows.into_iter().filter(|r| !(r.len() == 1 && r[0].trim().is_empty())).collect())
}
...
pub fn parse_date(raw: &str) -> Option<i64> {
    ...
    let local = |d: chrono::NaiveDateTime| d.and_local_timezone(chrono::Local).single().map(|x| x.timestamp());
    for fmt in ["%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%b %e, %Y %l:%M %p", "%b %e, %Y, %l:%M %p", "%m/%d/%Y %l:%M %p", "%m/%d/%Y %H:%M", "%d/%m/%Y %H:%M"] {
        if let Ok(d) = chrono::NaiveDateTime::parse_from_str(v, fmt) {
            return local(d);
        }
    }
...
fn parse_timecode(v: &str) -> Option<f64> {
    let parts: Vec<f64> = v.split(':').map(|p| p.trim().parse::<f64>().ok()).collect::<Option<Vec<_>>>()?;
    match parts.len() {
...
fn is_plausible_speaker(v: &str) -> bool {
    !v.is_empty()
        ...
        && v.chars().all(|c| c.is_alphanumeric() || c.is_whitespace() || "-_'.".contains(c))
}
...
fn is_generic_speaker(v: &str) -> bool {
    ...
    if ["me", "you", "myself", "them", "microphone", "system", "unknown"].contains(&l.as_str()) {
        return true;
    }
    let mut parts = l.split_whitespace();
    matches!(parts.next(), Some("speaker") | Some("participant")) && parts.clone().count() <= 1 && parts.all(|p| p.chars().all(|c| c.is_alphanumeric()))
}
...
fn speaker_metadata(raw: &str) -> (String, Option<String>, bool) {
    ...
    let generic = is_generic_speaker(v) || ["them", "speaker", "system", "unknown"].contains(&lower.as_str());
    (if lower.is_empty() { "unknown".into() } else { lower }, if generic { None } else { Some(v.to_string()) }, is_self)
}
...
fn parse_json_transcript(text: &str) -> Option<Vec<ImportedTurn>> {
    ...
    let get = |o: &serde_json::Map<String, serde_json::Value>, keys: &[&str]| keys.iter().find_map(|k| o.get(*k).cloned());
    let mut turns = Vec::new();
    for item in items {
        let Some(o) = item.as_object() else { continue };
        let text = get(o, &["text", "content", "transcript", "utterance"]).and_then(|v| v.as_str().map(String::from)).unwrap_or_default();
        if text.trim().is_empty() {
            continue;
        }
        let speaker_raw = get(o, &["speaker", "speakerName", "speaker_name", "name", "source"]).map(|v| v.as_str().map(String::from).unwrap_or_else(|| v.to_string())).unwrap_or_default();
        let (key, name, is_self) = speaker_metadata(&speaker_raw);
        let ts = get(o, &["timestamp", "start", "startTime", "start_time", "time", "offset"]);
        let (relative, absolute) = match ts {
            Some(serde_json::Value::Number(n)) => {
                let f = n.as_f64().unwrap_or(0.0);
                if f > 1_000_000_000_000.0 { (None, Some((f / 1000.0) as i64)) } else if f > 1_000_000_000.0 { (None, Some(f as i64)) } else { (Some(f), None) }
            }
        ...
        turns.push(ImportedTurn { text: text.trim().to_string(), speaker_key: key, speaker_name: name, is_self, relative_ts: relative, absolute_ts: absolute });
    }
    Some(turns)
}
...
pub fn parse_transcript(raw: &str) -> Vec<ImportedTurn> {
    ...
    for raw_line in trimmed.lines() {
        ...
        if let Some((speaker, text)) = split_speaker_prefix(&remainder) {
            ...
            turns.push(ImportedTurn { text, speaker_key: key, speaker_name: name, is_self, relative_ts: ts, absolute_ts: None });
            ...
            turns.push(ImportedTurn { text: remainder, speaker_key: "unknown".into(), speaker_name: None, is_self: false, relative_ts: ts, absolute_ts: None });
        }
    }
...
fn extract_email(v: &str) -> Option<String> {
    ...
    v.split(|c: char| c.is_whitespace() || c == '<' || c == '>' || c == ',' || c == ';' || c == '"' || c == '\'' || c == '(' || c == ')')
        .find(|t| {
            let Some(at) = t.find('@') else { return false };
            let (local, domain) = t.split_at(at);
            !local.is_empty() && domain.len() > 3 && domain[1..].contains('.') && domain[1..].chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-')
        })
...
fn extract_name(v: &str) -> Option<String> {
    ...
    nil_if_empty(v[..idx].trim_matches(|c: char| c.is_whitespace() || "<>\"'".contains(c)).to_string())
}

pub fn parse_attendees(raw: &str, owner: Option<&str>, organizer: Option<&str>) -> Vec<ImportedAttendee> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return Vec::new();
    }
    ...
    if let Ok(serde_json::Value::Array(items)) = serde_json::from_str::<serde_json::Value>(trimmed) {
        for item in items {
            if let Some(o) = item.as_object() {
                if let Some(email) = ["email", "emailAddress", "address"].iter().find_map(|k| o.get(*k).and_then(|v| v.as_str())) {
                    let name = ["name", "displayName"].iter().find_map(|k| o.get(*k).and_then(|v| v.as_str())).map(String::from);
    ...
    pairs
        ...
        .filter_map(|(name, email)| {
            ...
            Some(ImportedAttendee {
                ...
                is_organizer: organizer.map(|o| o.eq_ignore_ascii_case(&email)).unwrap_or(false),
                is_self: owner.map(|o| o.eq_ignore_ascii_case(&email)).unwrap_or(false),
                email,
            })
        })
        .collect()
...
fn imported_meeting(row: &[String], headers: &[String]) -> Option<ImportedMeeting> {
    ...
    let last_rel = transcript.iter().filter_map(|t| t.relative_ts).fold(None, |m: Option<f64>, x| Some(m.map_or(x, |y| y.max(x))));
    let end = start.max(row_end.or(last_abs).or(last_rel.map(|r| start + r as i64)).unwrap_or(start));
    let source_url = nil_if_empty(value(row, headers, URL_HEADERS));
    let raw_id = nil_if_empty(value(row, headers, ID_HEADERS));
    let external_id = raw_id.or_else(|| source_url.clone()).unwrap_or_else(|| stable_id(&title, start, &transcript_text));
    let owner = extract_email(&value(row, headers, OWNER_HEADERS));
    let organizer = extract_email(&value(row, headers, ORGANIZER_HEADERS));
    let attendees = parse_attendees(&value(row, headers, ATTENDEE_HEADERS), owner.as_deref(), organizer.as_deref());
...
pub fn parse(text: &str) -> Result<ParseResult, String> {
    ...
    let raw_headers = rows.first().filter(|r| !r.is_empty()).ok_or("This CSV does not contain recognizable Granola meeting columns.")?;
    let headers: Vec<String> = raw_headers.iter().map(|h| normalize_header(h)).collect();
    let recognized: HashSet<&str> = [ID_HEADERS, TITLE_HEADERS, START_HEADERS, END_HEADERS, SUMMARY_HEADERS, NOTES_HEADERS, TRANSCRIPT_HEADERS, ATTENDEE_HEADERS, OWNER_HEADERS, ORGANIZER_HEADERS, CALENDAR_HEADERS, URL_HEADERS]
        .iter()
    ...
    Ok(ParseResult { meetings, skipped_rows: skipped })
}
...
pub fn make_meeting(rec: &ImportedMeeting, default_language: &str) -> (Meeting, Vec<TranscriptSegment>) {
    ...
    for t in &rec.transcript {
        ...
        cursor = end;
        segments.push(TranscriptSegment::new(&m.id, id, t.text.clone(), start, end, if t.is_self { "microphone" } else { "system" }));
    }
    if !names.is_empty() {
...
mod tests {
    ...
    fn transcript_lines_with_timestamps_speakers_and_continuations() {
        let turns = parse_transcript("[00:01] Me: hello there\n(00:05) Alex Smith: hi\ncontinued line\nSpeaker 2: yo");
        ...
        assert_eq!(turns[2].speaker_name, None, "generic speaker labels carry no name");
        assert_eq!(turns[2].speaker_key, "speaker 2");
    }

    #[test]
    fn transcript_json_shape() {
        let turns = parse_transcript(r#"[{"speaker":"You","text":"hi","start":1.5},{"speaker":"Bob","text":"hey","timestamp":"2023-11-14T22:13:20Z"}]"#);
    ...
    fn attendees_from_json_and_strings() {
        let a = parse_attendees(r#"[{"name":"Al","email":"AL@acme.com"},"Bo <bo@x.io>"]"#, Some("al@acme.com"), Some("bo@x.io"));
        assert_eq!(a.len(), 2);
    ...
    fn full_parse_and_meeting_conversion() {
        ...
        assert!(segs[1].start_s >= segs[0].end_s, "sequential timestamps when none given");
    ...
    #[test]
    fn summary_message() {
        assert_eq!(ImportSummary { imported: 2, duplicates: 1, skipped_rows: 0 }.message(), "Imported 2 meetings · 1 already imported");
        assert_eq!(ImportSummary { imported: 0, duplicates: 0, skipped_rows: 1 }.message(), "No new meetings imported · 1 row skipped");
    }
}
