...
pub fn quiet_crossed_common_boundary(
    quiet_started_unix: i64,
    now_unix: i64,
    recording_secs: f64,
) -> bool {
    ...
    let Some(start) = local(quiet_started_unix) else {
        return false;
    };
...
pub fn should_offer_quiet_prompt(
    has_meaningful: bool,
    transcript_gap: f64,
    audio_gap: f64,
    ...
    let threshold = if crossed_boundary {
        120.0
    } else {
        smart_quiet_threshold_secs(auto_stop_minutes)
    };
...
pub fn can_auto_handoff(
    ...
    audio_gap: f64,
    auto_start: bool,
    calendar_auto_stop: bool,
    overlap: bool,
) -> bool {
    let (Some(ce), Some(ns)) = (current_end, next_start) else {
        return false;
    };
    auto_start
        && calendar_auto_stop
    ...
    pub fn from_clients(clients: Option<Vec<CaptureClient>>, at: Instant) -> Self {
        ...
        let mut active: Vec<ActiveCall> = clients
            ...
            .filter_map(|c| match classify(&c.app_name) {
                CallAppKind::Strong => Some(ActiveCall {
                    id: c.app_name.to_lowercase(),
                    name: c.app_name.clone(),
                    confidence: CallConfidence::Native,
                }),
                CallAppKind::Weak => Some(ActiveCall {
                    id: c.app_name.to_lowercase(),
                    name: c.app_name.clone(),
                    confidence: CallConfidence::Browser,
                }),
                CallAppKind::None => None,
            })
            .collect();
        active.sort_by(|a, b| {
            (a.confidence == CallConfidence::Browser)
                .cmp(&(b.confidence == CallConfidence::Browser))
                .then(a.id.cmp(&b.id))
        });
...
impl CallLifecycleEngine {
    fn end_debounce(c: CallConfidence) -> Duration {
        if c == CallConfidence::Browser {
            BROWSER_END_DEBOUNCE
        } else {
            END_DEBOUNCE
        }
    }
    fn end_quiet(c: CallConfidence) -> Duration {
        if c == CallConfidence::Browser {
            BROWSER_END_QUIET_REQUIREMENT
        } else {
            END_QUIET_REQUIREMENT
        }
    }
    ...
    pub fn ingest(
        ...
        if ctx.is_recording {
            self.ingest_recording(snapshot, ctx)
        } else {
            self.ingest_idle(snapshot)
        }
    }
    ...
    fn ingest_recording(
        ...
        if let Some(associated) = self.associated_app.clone() {
            if let Some(current) = snapshot.active.iter().find(|a| a.id == associated.id) {
                ...
                if self.end_candidate_since.is_some() {
                    self.end_candidate_since = None;
                    events.push(if ctx.in_ending_grace {
                ...
                match self.end_candidate_since {
                    Some(since) => {
                        if now.duration_since(since) >= Self::end_debounce(associated.confidence)
                            && ctx.transcript_gap >= Self::end_quiet(associated.confidence)
                        {
                            let ask_only = !ctx.has_meaningful_content
                                || ctx.recording_duration < SHORT_SESSION_MIN_DURATION;
                            events.push(LifecycleEvent::BeginEndingGrace {
                                app: associated.clone(),
                                ask_only,
                            });
                            self.end_candidate_since = None;
                        }
        ...
        for app in snapshot
            .active
            .iter()
            .filter(|a| Some(&a.id) != associated_id.as_ref())
        {
...
pub fn filler_rate_due(recent: &[YouSegment], window_end: f64) -> Option<f64> {
    ...
    let (words, fillers) = recent
        .iter()
        .filter(|s| s.end >= window_start)
        .fold((0usize, 0usize), |(w, f), s| (w + s.words, f + s.fillers));
...
impl MonitorState {
    ...
    pub fn tick(
            ...
            Some(Pending::EndingGrace { deadline }) => {
                ...
                if now >= deadline {
                    ...
                    let _ = app.emit(
                        "smart_prompt",
                        SmartPrompt {
                            id: "ending-grace".into(),
                            kind: "ending".into(),
                            message: format!("Call ended — saving in {left}s"),
                ...
                if activity.transcript_gap < 5.0 {
                    ...
                    let _ = app.emit(
                        ...
                        SmartPrompt {
                            ...
                            kind: "calendar".into(),
                            message: format!("Ending and starting “{}” in {left}s", event.title),
            ...
            Some(Pending::CalendarStart {
                ...
                if now >= deadline {
                    ...
                    let _ = app.emit(
                        "smart_prompt",
                        SmartPrompt {
                            id: format!("start-{}", event.id),
                            kind: "start".into(),
                            message: format!("Starting “{}” in {left}s", event.title),
                ...
                match ev {
                    LifecycleEvent::OfferStartPrompt(appc) => {
                        if self.pending.is_none() {
                            ...
                            self.set_pending(
                                app,
                                Pending::CallStart(appc),
                                SmartPrompt {
                                    id,
                                    kind: "start".into(),
                                    message: format!("{name} is using the microphone. Take notes?"),
                    ...
                    LifecycleEvent::BeginEndingGrace {
                        app: appc,
                        ask_only,
                    } => {
                        if ask_only {
                            let id = self.next_id("ending");
                            let title = if appc.confidence == CallConfidence::Browser {
                                "Browser call ended".to_string()
                            } else {
                                format!("{} call ended", appc.name)
                            };
                            self.set_pending(
                                app,
                                Pending::CallEndAsk(appc),
                                SmartPrompt {
                                    id,
                                    kind: "ending".into(),
                                    message: format!("{title}. Has this meeting finished?"),
                            ...
                            self.set_pending(
                                app,
                                Pending::EndingGrace { deadline },
                                SmartPrompt {
                                    id,
                                    kind: "ending".into(),
                                    message: format!(
                                        "{} call ended — saving in {}s",
                                        appc.name,
                                        ENDING_GRACE.as_secs()
                                    ),
                                    primary: "End now".into(),
                                    secondary: "Keep recording".into(),
                                    tertiary: None,
                    ...
                    LifecycleEvent::ReactivateDuringGrace => {
                        if matches!(
                            self.pending,
                            Some(Pending::EndingGrace { .. }) | Some(Pending::CallEndAsk(_))
                        ) {
                            self.clear_prompt(app);
                        }
                    }
                    LifecycleEvent::OfferTransition(appc) => {
                        if self.pending.is_none() {
                            ...
                            self.set_pending(
                                app,
                                Pending::Transition(appc),
                                SmartPrompt {
                                    id,
                                    kind: "calendar".into(),
                                    message: format!(
                                        "{name} call started. Switch to a new meeting?"
                                    ),
        ...
        if activity.recording {
            ...
            if prefs.auto_stop_minutes > 0 && !suppressed {
                ...
                if activity.meaningful_finals > 0
                    && activity.transcript_gap >= t
                    && activity.audio_gap >= t
                {
                    self.clear_prompt(app);
            ...
            if smart && self.pending.is_none() {
                let crossed = self
                    .quiet_started_at
                    .map(|q| {
                        quiet_crossed_common_boundary(
                            q,
                            activity.now_unix,
                            activity.recording_duration,
                        )
                    })
                    .unwrap_or(false);
                if should_offer_quiet_prompt(
                    activity.meaningful_finals > 0,
                    activity.transcript_gap,
                    activity.audio_gap,
                    prefs.auto_stop_minutes,
                    crossed,
                    self.quiet_prompted_this_episode,
                    suppressed,
                ) {
                    ...
                    self.set_pending(
                        ...
                        SmartPrompt {
                            ...
                            kind: "quiet".into(),
                            message: format!(
                                "No speech for {mins} minute{}. Has this meeting ended?",
                                if mins == 1 { "" } else { "s" }
                            ),
                            primary: "End meeting".into(),
            ...
            if smart
                && (self.pending.is_none()
                    || matches!(self.pending, Some(Pending::CalendarTransition { .. })))
            {
                ...
                if let Some(event) = next.cloned() {
                    ...
                    if !already {
                        ...
                        self.set_pending(
                            ...
                            SmartPrompt {
                                ...
                                kind: "calendar".into(),
                                message: format!(
                                    "Next: {}. Starts at {time}. Is the current meeting over?",
                                    event.title
                                ),
                                primary: "End & start next".into(),
                                secondary: "Keep recording".into(),
                    ...
                    if let Some(Pending::CalendarTransition {
                        event: e,
                        handoff_deadline: None,
                    }) = self.pending.clone()
                    {
                        let current =
                            current_event_id.and_then(|id| events.iter().find(|x| x.id == id));
                        let starts_in = e.start_ts().map(|s| s - activity.now_unix).unwrap_or(999);
                        let overlap = match (current.and_then(|c| c.end_ts()), e.start_ts()) {
                            (Some(ce), Some(ns)) => ns < ce,
                            _ => true,
                        };
                        if starts_in <= HANDOFF_COUNTDOWN as i64
                            && can_auto_handoff(
                                current.and_then(|c| c.end_ts()),
                                e.start_ts(),
                                activity.now_unix,
                                activity.transcript_gap,
                                activity.audio_gap,
                                prefs.calendar_auto_start,
                                prefs.calendar_auto_stop,
                                overlap,
                            )
                        {
                            self.pending = Some(Pending::CalendarTransition {
                                ...
                                handoff_deadline: Some(
                                    now + Duration::from_secs(
                                        starts_in.clamp(1, HANDOFF_COUNTDOWN as i64) as u64,
                                    ),
                                ),
                            });
                        }
            ...
            if self.pending.is_none() {
                for e in events
                    .iter()
                    .filter(|e| !e.is_all_day && !self.dismissed_events.contains(&e.id))
                {
                    ...
                    if prefs.calendar_auto_start && d > 0 && d <= 16 {
                        ...
                        self.set_pending(
                            app,
                            Pending::CalendarStart {
                                event: e.clone(),
                                deadline: Some(now + Duration::from_secs(d as u64)),
                            },
                            SmartPrompt {
                                id,
                                kind: "start".into(),
                                message: format!("Starting “{}” in {d}s", e.title),
                                primary: "Start now".into(),
                                secondary: "Don't start".into(),
                                tertiary: None,
                    ...
                    if !prefs.calendar_auto_start && (-120..=60).contains(&d) {
                        ...
                        self.set_pending(
                            ...
                            SmartPrompt {
                                id,
                                kind: "start".into(),
                                message: format!("“{}” is starting. Record it?", e.title),
                                primary: "Start recording".into(),
    ...
    pub fn evaluate_nudges(
        ...
        app: &AppHandle,
        prefs: &crate::prefs::Prefs,
        you_recent: &[YouSegment],
        you_run: &[YouSegment],
        high_questions: &[String],
        transcript_end: f64,
        now: Instant,
    ) {
        ...
        if let Some(q) = high_questions
            .iter()
            .find(|q| !self.notified_questions.contains(*q))
        {
            if self
                .last_question_nudge
                .map(|t| now.duration_since(t) >= QUESTION_NUDGE_MIN_INTERVAL)
                .unwrap_or(true)
            {
                self.notified_questions.insert(q.clone());
                self.last_question_nudge = Some(now);
                deliver(RecordingNudge {
                    id: format!("q-{}", self.notified_questions.len()),
                    kind: "question",
        ...
        if let Some(first) = you_run.first() {
            ...
            if monologue_due(you_run)
                && self.monologue_nudged_run_start != Some(run_start)
                && self
                    .last_monologue_nudge
                    .map(|t| now.duration_since(t) >= MONOLOGUE_NUDGE_MIN_INTERVAL)
                    .unwrap_or(true)
            {
                self.monologue_nudged_run_start = Some(run_start);
                self.last_monologue_nudge = Some(now);
                let secs =
                    (you_run.last().map(|s| s.end).unwrap_or(run_start) - run_start).round() as i64;
                deliver(RecordingNudge {
                    id: format!("m-{run_start}"),
                    kind: "monologue",
                    title: "Long stretch".into(),
                    message: format!(
        ...
        if let Some(rate) = filler_rate_due(you_recent, transcript_end) {
            if self
                .last_filler_nudge
                .map(|t| now.duration_since(t) >= FILLER_NUDGE_MIN_INTERVAL)
                .unwrap_or(true)
            {
                self.last_filler_nudge = Some(now);
                deliver(RecordingNudge {
                    id: format!("f-{}", transcript_end as i64),
                    kind: "filler",
                    title: "Filler burst".into(),
                    message: format!(
...
mod tests {
    ...
    fn call(id: &str, browser: bool) -> ActiveCall {
        ActiveCall {
            id: id.into(),
            name: id.into(),
            confidence: if browser {
                CallConfidence::Browser
            } else {
                CallConfidence::Native
            },
        }
    }
    ...
    fn ctx(recording: bool, gap: f64) -> LifecycleContext {
        LifecycleContext {
            ...
            recording_duration: Duration::from_secs(600),
    ...
    fn quiet_thresholds_follow_the_table() {
        ...
        assert!(should_offer_quiet_prompt(
            true, 300.0, 300.0, 0, false, false, false
        ));
        assert!(
            !should_offer_quiet_prompt(true, 300.0, 10.0, 0, false, false, false),
            "both transcript and audio must be quiet"
        );
        assert!(
            should_offer_quiet_prompt(true, 130.0, 130.0, 0, true, false, false),
            "boundary shortens to 2 min"
        );
        assert!(
            !should_offer_quiet_prompt(true, 400.0, 400.0, 0, false, true, false),
            "one prompt per episode"
        );
        assert!(
            !should_offer_quiet_prompt(true, 400.0, 400.0, 0, false, false, true),
            "suppressed"
        );
        assert!(
            !should_offer_quiet_prompt(false, 400.0, 400.0, 0, false, false, false),
            "needs content"
        );
    }
    ...
    fn common_boundary_rule() {
        ...
        assert!(
            !quiet_crossed_common_boundary(start, start + 60, 20.0 * 60.0),
            "needs ≥2 min quiet"
        );
        assert!(
            !quiet_crossed_common_boundary(start, start + 12 * 60, 10.0 * 60.0),
            "needs ≥15 min recording"
        );
        assert!(quiet_crossed_common_boundary(
            start,
            start + 12 * 60,
            20.0 * 60.0
        ));
        assert!(
            !quiet_crossed_common_boundary(start, start + 5 * 60, 20.0 * 60.0),
            "10:25 has not crossed :30"
        );
    }
    ...
    fn lifecycle_offers_start_after_debounce_and_clears() {
        ...
        let t0 = Instant::now();
        assert!(e
            .ingest(&snap(vec![call("zoom", false)], t0), ctx(false, 0.0))
            .is_empty());
        let evs = e.ingest(
            &snap(vec![call("zoom", false)], t0 + Duration::from_secs(3)),
            ctx(false, 0.0),
        );
        assert_eq!(
            evs,
            vec![LifecycleEvent::OfferStartPrompt(call("zoom", false))]
        );
        assert!(
            e.ingest(
                &snap(vec![call("zoom", false)], t0 + Duration::from_secs(4)),
                ctx(false, 0.0)
            )
            .is_empty(),
            "prompted once"
        ...
        assert!(e
            .ingest(&snap(vec![], t0 + Duration::from_secs(5)), ctx(false, 0.0))
            .is_empty());
        let evs = e.ingest(&snap(vec![], t0 + Duration::from_secs(9)), ctx(false, 0.0));
        assert!(
            evs.is_empty(),
            "prompt already dismissed → nothing to clear"
        );
        // A new call after the reset gap prompts again.
        assert!(e
            .ingest(
                &snap(vec![call("zoom", false)], t0 + Duration::from_secs(10)),
                ctx(false, 0.0)
            )
            .is_empty());
        let evs = e.ingest(
            &snap(vec![call("zoom", false)], t0 + Duration::from_secs(13)),
            ctx(false, 0.0),
        );
        assert_eq!(evs.len(), 1);
    }
    ...
    fn lifecycle_ending_grace_needs_debounce_and_quiet() {
        ...
        assert!(e
            .ingest(&snap(vec![], t0 + Duration::from_secs(1)), ctx(true, 1.0))
            .is_empty());
        // Debounce passed but transcript not quiet enough → no grace yet.
        assert!(e
            .ingest(&snap(vec![], t0 + Duration::from_secs(6)), ctx(true, 2.0))
            .is_empty());
        ...
        assert!(b
            .ingest(&snap(vec![], t0 + Duration::from_secs(7)), ctx(true, 6.0))
            .is_empty());
        assert_eq!(
            b.ingest(&snap(vec![], t0 + Duration::from_secs(10)), ctx(true, 9.0))
                .len(),
            1
        );
    }
    ...
    fn lifecycle_short_session_asks_and_reactivation_cancels() {
        ...
        let evs = e.ingest(
            &snap(vec![call("teams", false)], t0 + Duration::from_secs(8)),
            grace,
        );
    ...
    fn unreliable_snapshots_freeze_the_engine() {
        ...
        assert!(
            e.ingest(
                &snap(vec![call("zoom", false)], t0 + Duration::from_secs(1)),
                ctx(false, 0.0)
            )
            .is_empty(),
            "debounce restarted"
    ...
    fn snapshot_from_clients_prefers_native_apps() {
        let clients = vec![
            ...
            CaptureClient {
                app_name: "ZOOM VoiceEngine".into(),
                is_capturing: true,
            },
            CaptureClient {
                app_name: "parec".into(),
    ...
    #[test]
    fn nudge_rules() {
        let run: Vec<YouSegment> = (0..10)
            .map(|i| YouSegment {
                start: i as f64 * 7.0,
                end: i as f64 * 7.0 + 6.5,
                words: 20,
                fillers: 0,
            })
            .collect();
        ...
        assert!(
            filler_rate_due(&recent, 160.0).is_some(),
            "9/min over 60 words"
        );
        assert!(
            filler_rate_due(&recent[..1], 160.0).is_none(),
            "5/min is under threshold"
        );
        let few = vec![YouSegment {
            start: 100.0,
            end: 160.0,
            words: 10,
            fillers: 9,
