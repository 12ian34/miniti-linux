...
const MIGRATIONS: &[(&str, &str)] = &[
    ...
    ("meetings", "join_url TEXT"),
    ("meetings", "inferred_environment TEXT NOT NULL DEFAULT ''"),
];
...
pub struct Meeting {
    ...
    /// unknown | remote_likely | in_room_likely | hybrid_likely (see `smart::MeetingEnvironment`).
    #[serde(default)]
    pub inferred_environment: String,
    pub created_at: i64,
}
...
impl Meeting {
    pub fn new(title: impl Into<String>, language: impl Into<String>) -> Self {
        ...
        Self {
            ...
            join_url: None,
            inferred_environment: String::new(),
            import_source: None,
            pinned: false,
...
const MEETING_COLS: &str = "id,title,started_at,ended_at,language,notes,summary,action_items,\
    ...
    template_id,template_sections,created_at,join_url,inferred_environment";
    ...
    conn.execute(
        &format!(
           ...
           ON CONFLICT(id) DO UPDATE SET
             ...
             manual_speaker_ids=?26,investigations=?27,template_id=?28,template_sections=?29,join_url=?31,inferred_environment=?32"#
        ),
        params![
            ...
            m.join_url, m.inferred_environment,
        ],
    )?;
    Ok(())
}
...
fn row_to_meeting(row: &rusqlite::Row) -> DbResult<Meeting> {
    Ok(Meeting {
        ...
        join_url: row.get(30)?,
        inferred_environment: row.get(31)?,
    })
}
...
pub fn set_inferred_environment(conn: &Connection, id: &str, env: &str) -> DbResult<()> {
    conn.execute(
        "UPDATE meetings SET inferred_environment=?2 WHERE id=?1",
        params![id, env],
    )?;
    Ok(())
}
